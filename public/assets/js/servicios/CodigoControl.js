 ...
 */
import encryptMessageRC4 from "../servicios/AllegedRC4.js";
import convertBase64 from "../servicios/Base64SIN.js";
import generateVerhoeff from "../servicios/Verhoeff.js";
...
function generateControlCode(
    ...
    let string4 = dosageKey.substr(
        numbers[0] + numbers[1] + numbers[2],
        numbers[3]
    );
    let string5 = dosageKey.substr(
    ...
    var allegedRC4String = encryptMessageRC4(
        stringDKey,
        keyForEncryption,
        true
    );
...
function addVerhoeffDigit(value, max) {
    for (let i = 1; i <= max; i++) {
        let val = generateVerhoeff(value);
        value += val.toString();
    }
    return value;
}
...
export default generateControlCode;
